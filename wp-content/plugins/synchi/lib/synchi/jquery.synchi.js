...
if (!jQuery.concat) jQuery.extend({
    concat: function(a, b) { 
        var merged = {};
...
function synchi_call(action,params,onSuccess,onError,noJSON) {
    params = jQuery.concat(params,{synchi_action:action});
...
function synchi_showMessage(text,timeout) {
    var message = jQuery('<div class="synchi_mesageBox"><span>'+text+'</span></div>');
...
function synchi_hideMessage(callback) {
    var message = jQuery('.synchi_mesageBox');
...
jQuery(function(){
    ...
    jQuery.fn.synchi = function(file,onChange,owner,onFocus) {
        // define self
        ...
        // init CodeMirror
        var extension = jQuery(self.file.split('.')).last().get(0);
        switch(extension) {
        ...
        self.editor =  CodeMirror.fromTextArea(self.textarea,jQuery.concat(synchi_settings,{
            mode: self.mode,
            tabMode: "indent",
...
//                );
        ...
        self.save = function(success_callback,error_callback) {
            ...
            jQuery(self.textarea).text(self.editor.getValue());
