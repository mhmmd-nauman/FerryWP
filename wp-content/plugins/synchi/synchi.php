...
/*
  ...
  Description: A full IDE inside WP! Syntax highlighting + powerful IDE features in WP plugin editor, themes editor, text widgets and article HTML editor.
  Version: 5.1
  Author: Miloš Đekić
...
// define themes
$synchi_themes = array('default', 'ambiance', 'blackboard', 'cobalt', 'eclipse', 'elegant', 'erlang-dark', 'lesser-dark', 'monokai', 'neat', 'night', 'rubyblue', 'xq-dark', 'ide');
...
function synchi_echoCSSinclude($filepath)
{
    ?>
    <link rel="stylesheet"
          href="<?php echo WP_PLUGIN_URL; ?>/synchi/<?php echo $filepath; ?>.css?version=<?php echo SYNCHI; ?>"/> <?php
}
...
function synchi_echoJSinclude($filepath)
{
    ?>
    <script
        src="<?php echo WP_PLUGIN_URL; ?>/synchi/<?php echo $filepath; ?>.js?version=<?php echo SYNCHI; ?>"></script> <?php
}
...
function synchi_action_update_settings()
{
    ...
    if (!isset($_POST['synchi_option_lineWrapping'])) return;
    $lineWrapping = $_POST['synchi_option_lineWrapping'];
    if ($lineWrapping == 0 || $lineWrapping == 1) update_option('synchi_option_lineWrapping', $lineWrapping);
...
function synchi_request_handler()
{
    ...
    switch ($_REQUEST['synchi_action'])
    {
        case 'update_settings':
            synchi_action_update_settings();
            break;
        case 'get_editor_controls':
            synchi_action_get_editor_controls();
            break;
        case 'get_ide':
            synchi_action_get_ide();
            break;
        case 'get_file_contents' :
            synchi_action_get_file_contents();
            break;
        case 'serialize_tabs' :
            synchi_action_serialize_tabs();
            break;
        case 'save_file' :
            synchi_action_save_file();
            break;
        case 'create_file' :
            synchi_action_create_file();
            break;
        case 'create_folder' :
            synchi_action_create_folder();
            break;
        case 'delete_file' :
            synchi_action_delete_file();
            break;
        case 'paste_file' :
            synchi_action_paste_file();
            break;
        case 'file_tree':
            synchi_action_file_tree();
            break;
        default:
            return;
    }
...
function synchi_get_settings()
{
    ...
    return array(
        ...
        'lineWrapping' => get_option('synchi_option_lineWrapping') == 1,
    );
}
...
add_option('synchi_option_lineWrapping', 0);
