...
class PluginOptionsPanelModule {
	...
	function PluginOptionsPanelModule($args=array()){
		...
		if($this->registration && current_user_can($this->capability_license)){
			...
			new pop_plugin_registration(array('capability'=>$this->capability_license, 'plugin_id'=>$this->id,'open'=>false,'tdom'=>'pop','plugin_code'=>$this->notification->plugin_code,'options_varname'=>$this->options_varname,'api_url'=>$this->api_url));
		}
	...
	function plugins_api($false, $action, $arg){
		...
		if(is_object($slug) && property_exists($arg, 'slug') && $slug==$arg->slug){
...
<?php
	...
	function pop_notifications(){
		...
		$update_url = false===$this->pluginslug ? $r->url : admin_url('/update-core.php') ;
