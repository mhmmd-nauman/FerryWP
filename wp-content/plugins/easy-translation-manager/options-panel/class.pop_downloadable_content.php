...
class pop_downloadable_content {
	...
	function init(){
		wp_register_script('rh-dc', 	$this->module_url.'js/dc.js', array(), '2.4.3');
		wp_register_style('rh-dc', 		$this->module_url.'css/dc.css', array(), '2.4.3');
	}
		...
		if(false===$response){
			...
			if($response->R=='OK'){
				if($response->DC->type=='bundle'){
					...
					$e = new pop_importer(array('plugin_id'=>$this->plugin_id,'options_varname'=>$this->options_varname,'resources_path'=>$this->resources_path,'tdom'=>'pop','alt_temp'=>$this->alt_temp));
	...
	<div class="pop-dlc-item">
		...
		<div class="pop-dlc-details">
			...
			<div class="pop-discount-code">
				<input type="text" class="pop-discount-code-inp" value="" placeholder="<?php _e('Discount Code','pop')?>">
				<button type="button" class="btn btn-pop-discount-code"><?php _e('Apply Coupon','pop')?></button>
			</div>
