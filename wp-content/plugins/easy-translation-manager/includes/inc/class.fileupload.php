...
class etm_qqFileUploader {
    ...
    function __construct(array $allowedExtensions = array(), $sizeLimit = 10485760){        
        ...
        if (isset($_GET['qqfile'])) {
            $this->file = new etm_qqUploadedFileXhr();
        } elseif (isset($_FILES['qqfile'])) {
            $this->file = new etm_qqUploadedFileForm();
        } else {
            $this->file = false; 
        }
    }
     ...
     */
    function handleUpload( $replaceOldFile = TRUE){
		$uploadDirectory = ABSPATH . 'wp-content/';
		$uploadUrl =  trailingslashit(get_option('siteurl')) . 'wp-content/';

        if(!is_dir($uploadDirectory.'languages')){
            mkdir($uploadDirectory.'languages', 0777);
        }
        
        $uploadDirectory .= 'languages/';
        $uploadUrl .= 'languages/';
        ...
        if ($this->file->save($uploadDirectory . $filename . '.' . $ext)){
            return array(
				'success'=>true,
				'url'=>	'Upload complete ('.$filename . '.' . $ext . ')',
				'id'=> $_REQUEST['id']
			);
        } else {
