...
function  etm_tools_save_to_array($dir,$file) {
	...
	preg_match_all("/(".implode('|', $array_two_step).")\($ss_s$ss_ms(.*?)$ss_me$ss_s$ss_ms(.*?)$ss_mee/", $fh, $matche_2, PREG_SET_ORDER);	
	if(!empty($matche_2)){
		foreach($matche_2 as $matches_single){
	    	$default_string = $default_string2 = $default_placeholder = $mo_tag = '';
	    	
			if(!empty($matches_single['8']) && $matches_single['7'] != '$'){
				$mo_tag = $matches_single['8'];
			} else {
				$mo_tag = 'Variable textdomain';
			}
			
			if(!empty($matches_single['4']) && $matches_single['3'] != '$'){
				$default_string = $matches_single['4'];
			}

			etm_scaner_save_to_db($default_string,$default_string2,$default_placeholder,$mo_tag,$temp_file);
		}			
	}
	
	preg_match_all("/(".implode('|', $array_tree_step).")\($ss_s$ss_ms(.*?)$ss_me$ss_s$ss_ms(.*?)$ss_me$ss_s$ss_ms(.*?)$ss_mee/", $fh, $matche_3, PREG_SET_ORDER);
	if(!empty($matche_3)){
		foreach($matche_3 as $matches_single){
	    	$default_string = $default_string2 = $default_placeholder = $mo_tag = '';

			if(!empty($matches_single['12']) && $matches_single['11'] != '$'){
				$mo_tag = $matches_single['12'];
			} else {
				$mo_tag = 'Variable textdomain';
			}
			...
			if(!empty($matches_single['8']) && $matches_single['7'] != '$'){
				if($matche_n[1] == '_n_noop'){
					$default_string2 = $matches_single['8'];
				} else {
					$default_placeholder = $matches_single['8'];
				}
			}	
	...
	preg_match_all("/(".implode('|', $array_four_step).")\($ss_s$ss_ms(.*?)$ss_me$ss_s$ss_ms(.*?)$ss_me$ss_s(.*?)\,$ss_s$ss_ms(.*?)$ss_mee/", $fh, $matche_4, PREG_SET_ORDER);
	if(!empty($matche_4)){
		foreach($matche_4 as $matches_single){
	    	$default_string = $default_string2 = $default_placeholder = $mo_tag = '';
	    	
			if(!empty($matches_single['14']) && $matches_single['13'] != '$'){
				$mo_tag = $matches_single['14'];
			} else {
				$mo_tag = 'Variable textdomain';
			}
	...
	preg_match_all("/(".implode('|', $array_five_step).")\($ss_s$ss_ms(.*?)$ss_me$ss_s$ss_ms(.*?)$ss_me$ss_s$ss_ms(.*?)$ss_me$ss_s$ss_ms(.*?)$ss_me$ss_s$ss_ms(.*?)$ss_mee/", $fh, $matche_5, PREG_SET_ORDER);
	if(!empty($matche_5)){
		foreach($matche_4 as $matches_single){
	    	$default_string = $default_string2 = $default_placeholder = $mo_tag = '';

			if(!empty($matches_single['20']) && $matches_single['19'] != '$'){
				$mo_tag = $matches_single['20'];
			} else {
				$mo_tag = 'Variable textdomain';
			}
			...
			if(!empty($matches_single['16']) && $matches_single['15'] != '$'){
				$default_placeholder = $matches_single['16'];
			}	
...
function etm_scaner_save_to_db($default_string = '',$default_string2 = '',$default_placeholder = '',$mo_tag = '', $temp_file = ''){
	if(!empty($default_string) && strpos($mo_tag,"'") == false && strpos($mo_tag,'"') == false){
	   global $wpdb,$userdata,$etm_tag,$etm_folder,$_etm_count;
	   
	   $user_count = $wpdb->get_row("SELECT id,mo_tag,folder_name,default_string2,default_placeholder FROM {$wpdb->prefix}etm_plugin_index WHERE BINARY default_string='".$default_string."' and BINARY default_string2='".$default_string2."' and default_placeholder='".$default_placeholder."' and BINARY default_placeholder='".$default_placeholder."' and mo_tag='".$mo_tag."' and category_type ='".$etm_tag."' and folder_name='".$etm_folder."' LIMIT 1");
        
        if(empty($user_count)){
	        if(!empty($default_string) && strpos($default_string,"'")>0 && strpos($default_string,"\\'") == false){
				  $default_string = str_replace("'","\'",$default_string);
	        }
	        
	        if(!empty($default_string2) && strpos($default_string2,"'")>0 && strpos($default_string2,"\\'") == false){
				  $default_string2 = str_replace("'","\'",$default_string2);
	        }
            $sql = "INSERT INTO {$wpdb->prefix}etm_plugin_index (default_string,default_string2,default_placeholder,folder_name,mo_tag,category_type,file,manual_added,create_user,create_ip) 
            	  VALUES ('".$default_string."','".$default_string2."','".$default_placeholder."','".$etm_folder."','".$mo_tag."','".$etm_tag."','".$etm_folder.'/'.$temp_file[1]."','0',".$userdata->ID.",'".$_SERVER['REMOTE_ADDR']."');";
            	  $wpdb->query($sql);      
