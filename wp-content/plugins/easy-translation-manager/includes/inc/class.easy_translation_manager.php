...
class easy_translation_manager_plugin { 
	...
	function easy_translation_manager_plugin( $args=array() ){
    	...
    	if(!empty($this->etm_tools_retrive['change_wp_admin'])){
   		...
   		if((!empty($tmp_change_wp_admin) && $tmp_change_wp_admin) || !is_admin()){
		...
		//This has to be loaded also in non-admin for correct interoperability.
		require_once EASY_TRANSLATION_MANAGER_OPTIONPANEL.'options-panel/load.pop.php';
		rh_register_php('options-panel',EASY_TRANSLATION_MANAGER_OPTIONPANEL.'options-panel/class.PluginOptionsPanelModule.php', '2.3.2');
		rh_register_php('rh-functions', EASY_TRANSLATION_MANAGER_OPTIONPANEL.'options-panel/rh-functions.php', '2.3.2');	
    ...
    function plugins_loaded(){
		...
		if(class_exists( 'WooCommerce' )){
			...
			add_action('woocommerce_init', array(&$this,'woocommerce_cart_totals_before_shipping'),9999999999, 1);
			add_filter('woocommerce_cart_item_product', array(&$this,'woocommerce_cart_item_product'),2, 1000);
			add_action('registered_taxonomy', array(&$this,'wp_get_object_terms'),2);
			add_filter('woocommerce_attribute_label', array(&$this,'woocommerce_get_item_data'),1);
		}
    ...
    function woocommerce_get_item_data($name = '', $extra = ''){
		return __($name,'etm');
    }
    ...
    function wp_get_object_terms ($terms = '', $object_ids = '', $taxonomies = '', $args = '' ){
	    ...
	    if(!empty($terms) && !empty($wp_taxonomies[$terms])){
		    $wp_taxonomies[$terms]->label = __($wp_taxonomies[$terms]->label,'etm');
		    $wp_taxonomies[$terms]->labels->name = __($wp_taxonomies[$terms]->labels->name,'etm');
		    $wp_taxonomies[$terms]->labels->singular_name = __($wp_taxonomies[$terms]->labels->singular_name,'etm');   
	    }
    }
    ...
    function aktivate_langushes(){
		...
		if($this->show_lang_status && (!empty($tmp_change_wp_admin) || !is_admin() || (!empty($_POST['action']) && strpos($_POST['action'],'woocommerce') !== false && is_admin() ))){
    ...
    function single_term_title($before = ''){
		...
		$translatede_tmp = $this->tran_terms_data[$tag->term_id];
 		...
 		if(!empty($translatede_tmp)){
 			if(!empty($this->etm_tools_retrive['use_permalink'])){
 				return $translatede_tmp;
 			} else {
 				if(!empty($before->slug)){
	 				$translatede_tmp->slug = $before->slug;
 				}
 				return $translatede_tmp;
 			}
 		} else {
	...
	function woocommerce_cart_totals_before_shipping($test='',$test2=''){
		...
		$packages = WC()->payment_gateways->payment_gateways;
		if(!empty($packages)){
			foreach($packages as $key_step1 => $data_step1){
				if(!empty($data_step1->title)){
					$packages[$key_step1]->title = __($data_step1->title,'woocommerce');
				}
				
				if(!empty($data_step1->description)){
					$packages[$key_step1]->description = __($data_step1->description,'woocommerce');
				}
			}
			WC()->payment_gateways->payment_gateways = $packages;
		}
		return $test;
